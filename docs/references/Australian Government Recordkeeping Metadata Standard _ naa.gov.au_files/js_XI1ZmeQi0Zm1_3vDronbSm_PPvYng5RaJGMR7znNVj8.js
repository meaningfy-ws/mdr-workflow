 ...
 *
 * Commercial license purchased for National Archives website by Oxide Interactive 2018
 */
...
(function($, window, document, undefined) {
  ...
  function objectFit() {
    if('objectFit' in document.documentElement.style === false) {
      $('.img-cover').not('.img-cover-processed').not('.img-cover-ie').each(function() {
  ...
  // Much nicer wrapping for titles, to avoid widows (single word on final line)
  function noWidow() {
    $('.no-widow').not('.no-widow-processed').each(function() {
      var words = $(this).text().split(' ');
      if (words.length > 2) {   // More than 2 words
        // Using a CSS instead of &nbsp; so that we can disable on mobile
        words[words.length-2] = '<span class="no-wrap">' + words[words.length-2] + ' ' + words[words.length-1] + '</span>';
        words.pop();
        $(this).html(words.join(' '));
        $(this).addClass('no-widow-processed');
      }
    });
  }

  // Simple keyboard tweak to make the main menu more accessible for keyboard users
  // This assumes the links under the megamenu are available on the landing pages too
  function menuKey() {
    $('.primary-menu').not('.menu-key-processed').each(function() {
      var $menu = $(this);
      var $links = $menu.find('.nav-link');
      var menuTimeout;
      function closeMenu() {
        $menu.find('.hover').attr('aria-expanded', false).removeClass('hover');
      }
      // Open and close the submenu
      $links.bind('keydown.menukey', function(e) {
        var key = e.which;
         // 32: Space, 40: ArrowDown
        if (key == 32 || key == 40) {
          $(this).parent().attr('aria-expanded', true).addClass('hover');
          e.preventDefault();
          return false;
        }
        // 38: ArrowUp, 27: Escape
        if (key == 38 || key == 27) {
          closeMenu();
        }
      });
      // Collapse when tabbing to a new menu item
      $links.parent().find('a').bind('focusout.menukey', function() {
        menuTimeout = window.setTimeout(closeMenu, 250);
      });
      $links.parent().find('.menu a').bind('focusin.menukey', function() {
        window.clearTimeout(menuTimeout);
      });
      $(this).addClass('menu-key-processed');
    });
  }
  ...
  function searchToggle() {
    $('.search-toggle').click(function(){
      ...
      $('body').addClass('search-toggle-open');
	 
    });
    $('.search-toggle-close').click(function(){
      $('body').removeClass('search-toggle-open');
    });
  }
  ...
  function searchQueryFromURL() {
	...
	if($('#search').length > 0)
	{
		...
		a = a.replace(/[+]/g,' ');   //replace plus with whitespace
  ...
  // Toggle for menu
  function menuToggle() {
    // Create offscreen menu (Mmenu)
    var $icon = $('#offscreen-menu');
    var $menu = $('#offscreen');
    ...
    // Add classes on accessible <button>
  ...
  function closeButton() {
    var $alert = $(".close-button a");
    $alert.on('click', function(){
      $alert.parents(".block-announcement-top").css("display","none");
    });
  }

  //matching height
  function matchHeights() {
    $('.featured-item').matchHeight({ property: 'min-height' });
    $('.action-item').matchHeight({ property: 'min-height' });
    $('.action-item-action-thumb').matchHeight({ property: 'min-height' });
    $('.action-item-action-thumb-summary').matchHeight({ property: 'min-height' });
    $('.action-item-feature-thumb').matchHeight({ property: 'min-height' });
    $('.action-item-feature-thumb-summary').matchHeight({ property: 'min-height' });
    $('.action-item-summary').matchHeight({ property: 'min-height' });
    $('.featured-item-action-thumb').matchHeight({ property: 'min-height' });
    $('.featured-item-action-thumb-summary').matchHeight({ property: 'min-height' });
    $('.featured-item-feature-thumb-summary').matchHeight({ property: 'min-height' });
    $('.featured-item-summary').matchHeight({ property: 'min-height' });
    $('.featured-item-feature-thumb').matchHeight({ property: 'min-height' });
    $('.featured-item-feature-thumb-summary-all-match').matchHeight({ byRow: false, property: 'min-height' });
    
    $('.view-events-upcoming .node.node--view-mode-condensed-event-item').matchHeight({ property: 'min-height' });
    $('.view-events-upcoming .node.node--view-mode-event-item').matchHeight({ property: 'min-height' });
  }

  function shaveText() {
    $('.carousel-item .field-description .field-title').shave(25);
    $('.carousel-item .field-description .field-date').shave(70);
  }

  

  /*function eventDateSpans() {
    var allDateItems = document.querySelectorAll('.view-events .views-row .field-date');
    allDateItems.forEach(function(fieldDate){

      //Adding from and to spans
      var fieldDateA = fieldDate.firstChild;
      var fieldDateAString = fieldDate.firstChild.nodeValue;
      fieldDate.firstChild.nodeValue = fieldDateAString.trim();
      var fieldDateB = fieldDateA.splitText(6);

      var spanA = document.createElement('span');
      spanA.className = "date date-from";
      fieldDate.insertBefore(spanA,fieldDateA);
      spanA.appendChild(fieldDateA);

      var spanB = document.createElement('span');
      spanB.className = "date date-to";
      fieldDate.insertBefore(spanB,fieldDateB);
      spanB.appendChild(fieldDateB);

      var spanC = document.createElement('span');
      spanC.className = "element-invisible";
      spanC.innerText = " to ";
      fieldDate.insertBefore(spanC, spanB);

      //Adding day and month spans
      var spanA1String = spanA.firstChild;
      var spanA2String = spanA1String.splitText(2);

      var spanDay = document.createElement('span');
      spanDay.className= "date-day";
      spanA.insertBefore(spanDay,spanA1String);
      spanDay.appendChild(spanA1String);

      var spanMonth = document.createElement('span');
      spanMonth.className= "date-month";
      spanA.insertBefore(spanMonth,spanA2String);
      spanMonth.appendChild(spanA2String);

      var spanB1String = spanB.firstChild;
      var spanB1StringValue = spanB1String.nodeValue;
      spanB1String.nodeValue = spanB1StringValue.trim()
      var spanB2String = spanB1String.splitText(2);

      var spanDayB = document.createElement('span');
      spanDayB.className= "date-day";
      spanB.insertBefore(spanDayB,spanB1String);
      spanDayB.appendChild(spanB1String);

      var spanMonthB = document.createElement('span');
      spanMonthB.className= "date-month";
      spanB.insertBefore(spanMonthB,spanB2String);
      spanMonthB.appendChild(spanB2String);
    });

    
  }*/

  function alertLinkClassAdd(){
    if($('.announcement-link')[0]){
      $('.notice.alert').addClass('has-link');
    }
  }
...
(function ($, Drupal) {
  Drupal.behaviors.matchHeightBehavior = {
    attach: function (context, settings) {
      $('.featured-item').matchHeight({ remove: true });
        $('.action-item').matchHeight({ remove: true });
        $('.action-item-action-thumb').matchHeight({ remove: true });
        $('.action-item-action-thumb-summary').matchHeight({ remove: true });
        $('.action-item-feature-thumb').matchHeight({ remove: true });
        $('.action-item-feature-thumb-summary').matchHeight({ remove: true });
        $('.action-item-summary').matchHeight({ remove: true });
        $('.featured-item-action-thumb').matchHeight({ remove: true });
        $('.featured-item-action-thumb-summary').matchHeight({ remove: true });
        $('.featured-item-feature-thumb-summary').matchHeight({ remove: true });
        $('.featured-item-summary').matchHeight({ remove: true });
        $('.featured-item-feature-thumb').matchHeight({ remove: true });
        $('.featured-item-feature-thumb-summary-all-match').matchHeight({ remove: true });
        
        $('.view-events-upcoming .node.node--view-mode-condensed-event-item').matchHeight({ remove: true });
        $('.view-events-upcoming .node.node--view-mode-event-item').matchHeight({ remove: true });

        //add matching with updated elements
        $('.featured-item').matchHeight({ property: 'min-height' });
        $('.action-item').matchHeight({ property: 'min-height' });
        $('.action-item-action-thumb').matchHeight({ property: 'min-height' });
        $('.action-item-action-thumb-summary').matchHeight({ property: 'min-height' });
